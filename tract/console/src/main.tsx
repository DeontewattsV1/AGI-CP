...
const agents: Agent[] = [
  { id: "human", name: "HUMAN", state: "normal", x: 50, y: 10, cap: ["root.authority"] },
  { id: "0001", name: "orchestrator", parent: "human", state: "normal", x: 50, y: 30, cap: ["delegate", "repo.read"] },
  { id: "0011", name: "researcher", parent: "0001", state: "normal", x: 28, y: 53, cap: ["repo.read"] },
  { id: "0012", name: "coder", parent: "0001", state: "normal", x: 50, y: 53, cap: ["repo.read", "git.write"] },
  { id: "0013", name: "analyst", parent: "0001", state: "normal", x: 72, y: 53, cap: ["repo.read"] },
  { id: "0021", name: "browser", parent: "0011", state: "normal", x: 17, y: 79, cap: ["http.request"] },
  { id: "0022", name: "shell", parent: "0012", state: "quarantine", x: 38, y: 79, cap: ["shell.exec"] },
  { id: "0023", name: "git", parent: "0012", state: "restrict", x: 50, y: 79, cap: ["git.write"] },
  { id: "0024", name: "mcp", parent: "0013", state: "observe", x: 62, y: 79, cap: ["mcp.tool"] },
  { id: "xxxx", name: "external", parent: "0013", state: "terminate", x: 83, y: 79, cap: [] },
];
...
function Lineage() {
  ...
  return <div className="lineage" aria-label="Synthetic agent lineage demonstration">
    <div className="sun" aria-hidden="true">𓂀</div>
    ...
    {agents.filter((agent) => agent.parent).map((agent) => {
      ...
      return <svg className="edge" key={`edge-${agent.id}`} aria-hidden="true">
        <line x1={`${parent.x}%`} y1={`${parent.y}%`} x2={`${agent.x}%`} y2={`${agent.y}%`}
          stroke={colors[agent.state]} strokeWidth="2" opacity=".72" />
      </svg>;
    })}
...
function Mosaic() {
  const glyphs = ["𓂀", "𓋹", "𓆣", "𓏞", "𓍿"];
  return <div className="mosaic" aria-label="Synthetic signed event receipt mosaic">
    {Array.from({ length: 96 }, (_, i) => <span key={i} className={`tile t${i % 5}`}>{glyphs[i % glyphs.length]}</span>)}
  </div>;
}
...
function App() {
  ...
  return <main>
    <header>
      <div className="wing" aria-hidden="true">𓅃</div>
      ...
      <div className="ankh" aria-hidden="true">☥</div>
    </header>
    ...
    <section className="bottom">
      <Panel title="GEOMETRIC INSIGHTS"><div className="insight"><div className="sigil">◈</div>
      ...
      <Panel title="AGENT PASSPORT (SELECTED)"><dl>
        ...
        <dt>purpose</dt><dd>develop features</dd><dt>capabilities</dt><dd>[repo.read, git.write]</dd>
        <dt>status</dt><dd className="valid">VALID</dd>
      </dl></Panel>
      <Panel title="QUICK INJECTION / PATCH"><div className="patches">
        {["Update Policy", "Revoke Agent", "Inject Guardrail", "Deploy Rule", "Emergency Stop"].map((name) =>
          <button onClick={() => setPatch(name)} key={name}>{name}</button>)}
        {patch && <small>Request staged: {patch}. Authenticated control API + signed receipt required.</small>}
      </div></Panel>
    </section>

    <footer>TRACT v0.2　𓂀　 PAST IS PROOF · PRESENT IS PROTECTION · FUTURE IS ALIGNMENT　∞</footer>
  </main>;
}
